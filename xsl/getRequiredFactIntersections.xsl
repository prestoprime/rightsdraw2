...
<xsl:template match="owl:Ontology">
	<xsl:apply-templates select="owl:ObjectPropertyAssertion[substring-after(owl:ObjectProperty/@IRI,'#')='hasRequired']" />
</xsl:template>
...
<xsl:template match="owl:ObjectPropertyAssertion">
	<xsl:if test="owl:NamedIndividual[position()=1]/@IRI = $permission or $permission='null'">
		...
		<xsl:variable name="count"><xsl:value-of select="count(../owl:ObjectPropertyAssertion[owl:NamedIndividual[position()=2]/@IRI=$ind])"/></xsl:variable>
		<xsl:if test="$count = 1">
...
<xsl:template match="owl:ClassAssertion">
	<xsl:if test="substring-after(owl:Class/@IRI,'#')='FactIntersection'">
