...
<xsl:template match="owl:AnnotationAssertion">
	<xsl:variable name="okey"><xsl:value-of select="owl:AnnotationProperty/@IRI"/></xsl:variable>
	<xsl:variable name="oowner"><xsl:value-of select="owl:IRI/text()"/></xsl:variable>
	<xsl:variable name="ovalue"><xsl:value-of select="owl:Literal/text()"/></xsl:variable>
	<xsl:if test="$key != $okey or $owner != $oowner or $value != $ovalue">
		<xsl:copy>
			<xsl:apply-templates select="@*|node()" />
		</xsl:copy>
	</xsl:if>
